...
| `No profiles for 'com.arrunner.phone' were found` + `Your team has no devices…` | Xcode's automatic signing tried to mint an *iOS App Development* profile (which needs registered devices) instead of an App Store profile. Fixed in the workflow by pinning `CODE_SIGN_IDENTITY[sdk=iphoneos*] = Apple Distribution` (and the `watchos*` variant) into `Config/Signing.xcconfig` before `xcodegen generate`. If you ever see this again, confirm those lines are still being appended in `release-testflight.yml` → "Write Config/Signing.xcconfig" step. | Workflow fix — no action needed in the portal. |
| `has conflicting provisioning settings ... is automatically signed, but code signing identity ... has been manually specified` | Someone re-introduced `CODE_SIGN_IDENTITY=...` on the `xcodebuild` command line. xcodebuild's CLI parser doesn't accept the `[sdk=...]` conditional form, and pinning the bare key while `CODE_SIGN_STYLE=Automatic` is rejected as a conflict by every target. | Move the override back into `Config/Signing.xcconfig` (xcconfig values are project config, not "manually specified") and remove it from the xcodebuild command line. See rc2 failure run 25990326363. |
