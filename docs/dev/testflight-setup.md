...
| `No profiles for 'com.arrunner.phone' were found` + `Your team has no devices…` | Xcode's automatic signing tried to mint an *iOS App Development* profile (which needs registered devices) instead of an App Store profile. Fixed in the workflow by pinning `CODE_SIGN_IDENTITY="Apple Distribution"` on the archive command. If you ever see this again, confirm that override is still present in `release-testflight.yml`. | Workflow fix — no action needed in the portal. |
