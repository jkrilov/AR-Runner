...
### 2026-05-17T21:54Z — rc5 archive trap: manual signing engaged, but provisioning profiles lack required capabilities because the App IDs in the developer portal don't declare them

**Context.** v0.2.0-rc5 (run [26004285341](https://github.com/jkrilov/AR-Runner/actions/runs/26004285341)) confirmed the rc4 fix worked — the "automatically signed for development" conflict is gone. Archive now fails one layer deeper with two new errors:
...
**Root cause.** `-allowProvisioningUpdates` + the ASC API key mints Apple Distribution profiles on demand, but only for capabilities **the App ID in the Apple Developer portal already declares**. The repo entitlements (`Config/ARRunnerPhone.entitlements` → App Groups + HealthKit; `Config/ARRunnerWidgetsPhone.entitlements` → App Groups) and Info.plist usage strings are all correct. The bundle IDs `com.arrunner.phone` and `com.arrunner.phone.widgets` simply don't have App Groups / HealthKit ticked on the App ID record on developer.apple.com. No code change can fix this — it's a portal action by the Account Holder.

**Strategy: Path (a).** Joe enables the capabilities on both App IDs (and registers `group.com.arrunner.shared` if absent), then retags rc6. Rejected: path (b) explicit `PROVISIONING_PROFILE_SPECIFIER` per target — requires both portal work *and* CI coupling to profile names, with no upside over path (a). Path (c) fastlane `match` — out of scope; pulling it in to dodge a portal toggle would be massive over-engineering.

**Trade-off named.** The source of truth for App ID capabilities lives outside the repo, invisible to code review. Mitigation: SKILL.md now names this as a distinct trap class with a pre-flight runbook step, and the TestFlight setup doc should grow a "portal prerequisites" checklist before the next rc.

**Meta-observation (fifth iteration).** Every previous rc was a *code/config* problem masked as a signing problem; rc5 is the inverse — a genuine portal problem the code can't see. The PR-time `-showBuildSettings` probe (D-RICHARDS-TF-8) wouldn't have caught it. What would have caught it: a one-time "check App ID capabilities match entitlements" pre-flight, done once per new capability added to an entitlements file. That's runbook discipline, not automation — and it belongs in the TestFlight setup doc as a checklist gate before tagging any rc that introduces a new entitlement.

**Durable rule added to skill (rc5 CRITICAL TRAP section).** "Manual-signed CLI archive errors of the form `<Target> requires a provisioning profile with the <Capability> feature` mean the App ID in the developer portal does not have that capability enabled. Fix in the portal; do not touch the workflow." Confidence: high (five-rc chain, each iteration with a distinct terminal root cause).

**Artifacts.**
- Decision: `.squad/decisions/inbox/richards-tf-rc5-provisioning-entitlements.md` (D-RICHARDS-TF-11).
- Skill update: new "rc5 CRITICAL TRAP" section + last-updated bump.
- No PR — explicitly *not* a code-fixable issue. Sending Joe a precise portal-action ask instead.
