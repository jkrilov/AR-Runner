...
**Fix — app-extension schemes (the "cheapest if you accept the trade", and in practice often the only thing that works).** Drop the standalone matrix entry for the extension. The parent watch app's `embed: true` dependency forces the extension to compile transitively whenever the watch app builds. You lose per-extension build telemetry but save 3–5 min per PR and one matrix job's macOS minutes.

**Fix — app-extension schemes (heavyweight, sometimes doesn't even work).** Install the watchOS Simulator runtime explicitly and keep `=watchOS Simulator`:
...
**Caveat (observed on Xcode 26.4 / macos-26-arm64):** `xcodebuild -downloadPlatform watchOS` installs the platform runtime ("watchOS 26.4 is already downloaded as universal") but **does NOT cause watchOS Simulator destinations to appear** in `xcodebuild -showdestinations` for app-extension schemes specifically. The destination list still shows only macOS / iOS / iOS Simulator / visionOS Simulator. Likely related to the scheme's supported-platforms metadata generated by xcodegen for shared-source widget extensions; root-causing it is rarely worth the time. Prefer the "drop the matrix entry" approach above when the extension is already embedded by a parent app target you do build standalone.
